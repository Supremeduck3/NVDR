 ...
 *
 *   [header 16B]  "NVDA", version 1, flags, image count u32, 6 reserved
 *   per image:    name length u16, name (UTF-8, at most 255 bytes),
 *                 container length u32, an NVDR container
 *
 ...
 * on because it costs nothing either.
 *
 ...
 * image's container size, and `cold_out` what it would have been without
 * the context (encoded a second time, for the report). */
...
int  nvda_next(NvdaReader* r, char* name, size_t cap, NvdrImage* out, NvdrHeader* hdr,
               int* partial);
